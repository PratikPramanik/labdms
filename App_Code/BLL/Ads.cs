...
namespace AspNet.StarterKits.Classifieds.BusinessLogicLayer
{
    ...
    public sealed class AdsDB 
    {
        ...
        #region Inserting Forms
        public static int InsertAd(int memberId, int categoryId, string title, string description, 
            ...
            string code, string syn1, string syn2, string syn3, string syn4, string syn5)
        {
            ...
            {
                adId = (int) db.InsertAd(memberId, categoryId, title, description, url, price, 
                    location, expirationDate, dateCreated, dateApproved, numViews, numResponses, 
                    (int)adLevel, (int)adStatus, (int)adType, code, syn1, syn2, syn3, syn4, syn5);
        ...
        public static AdsDataComponent.AdsDataTable GetActiveAdsByQuery(int recordLimit, int categoryId, 
            int memberId, decimal maxPrice, string searchTerm, string location, int adType, int adLevel, 
            int dayRange, bool mustHaveImage
            /*, string code , string syn1, string syn2, string syn3, string syn4, string syn5*/)
        {
        ...
        public static AdsDataComponent.AdsDataTable GetAdsByQuery(int recordLimit, int categoryId, 
            int memberId, decimal maxPrice, string searchTerm, string location, int adType, 
            int adStatus, int adLevel, int dayRange, bool mustHaveImage
            /*, string code , string syn1, string syn2, string syn3, string syn4, string syn5*/)
        {
            ...
            using (AdsDataAdapter db = new AdsDataAdapter())
            {
                ...
                aa = db.GetAllAdsByQuery(recordLimit, categoryId, memberId, maxPrice, 
                    searchTerm, location, adType, adStatus, adLevel, minCreatedDate, mustHaveImage
                    /*, code , syn1, syn2, syn3, syn4, syn5*/);
                return aa;
            }
        }
        ...
        #region Update Forms
        public static void UpdateAd(int original_Id, int memberId, string title, string description, 
            string url, decimal price, string location, bool isRelisting,
            string code, string syn1, string syn2, string syn3, string syn4, string syn5)
        {
            if (url == null)
                url = String.Empty;
            using (AdsDataAdapter db = new AdsDataAdapter())
            {
                db.UpdateAd(original_Id, memberId, 
                    ...
                    HttpUtility.HtmlEncode(syn1),
                    HttpUtility.HtmlEncode(syn2),
                    HttpUtility.HtmlEncode(syn3),
                    HttpUtility.HtmlEncode(syn4),
                    HttpUtility.HtmlEncode(syn5));
            }
        }
        ...
        {
            /* //to prevent forms from being expired this method is commented out.
            using (AdsDataAdapter db = new AdsDataAdapter())
            {
        ...
        public static bool SendAdInEmail(int adId, string senderName, string senderAddress, 
            string recipientEmail, string subject, string message)
        {
            bool sent = false;
            string from = String.Format("{0} <{1}>", senderName, "user@labdms.scvmc");
            //string from = String.Format("{0} <{1}>", senderName, senderAddress); //old code, now hardcoded

            //check if there is no inputted email
            string email;
            if (senderAddress.Equals("") || senderAddress == null) { email = "N/A"; }
            else { email = senderAddress; }

            //attach persons email
            string appendedemail = String.Format("\nRespond to {0} at: ", senderName, email);

            try
            {
                MailMessage m = new MailMessage(from, recipientEmail);
                m.Subject = subject;
                m.Body = message + appendedemail;
                //m.Body = message;
        ...
        public static bool SendResponse(int adId, string senderName, string senderAddress, string comments)
        {
            ...
            if (ad != null)
            {
                ...
                if (adUser != null)
                {
                    ...
                    sb.AppendFormat("A user has sent a response to your Form '{0}' at {1}", ad.Title, s.SiteName);
                    sb.AppendLine();
                    ...
                    try
                    {
                        ...
                        m.Subject = "Response for Form: " + ad.Title;
                        m.Body = sb.ToString();
                        SmtpClient client = new SmtpClient();
                        client.Send(m);
