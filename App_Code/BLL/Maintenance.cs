...
namespace AspNet.StarterKits.Classifieds.BusinessLogicLayer
{
    ...
    public sealed class Maintenance
    {
        ...
        public static void SendAdNotification(AdsDataComponent.AdsRow ad)
        {
            ...
            messageBody.AppendFormat("A new Form has just been to FormSearch SCVMC '{0}':", s.SiteName);
            ...
            {
                messageBody.AppendLine("Status: The Form is not yet active.");
            }
            else
            {
                messageBody.AppendLine("Status: The Form was activated automatically.");
            }
            ...
            try
            {
                ...
                m.Subject = String.Format("New Form posted: {0}", ad.Title);
        ...
        private static void SendSummaryMail(int numPendingNew, int numPendingTotal)
        {
            ...
            if (numPendingNew > 0 || (s.AdminNotification == AdminNotificationSetting.Daily && numPendingTotal > 0))
            {
                ...
                messageBody.AppendLine  ("There are new Forms waiting for activation:");

                messageBody.AppendLine();

                messageBody.AppendFormat("... {0} new Forms since the last notification was sent", numPendingNew);
                messageBody.AppendLine();

                messageBody.AppendFormat("... {0} total Forms waiting for activation", numPendingTotal);
                messageBody.AppendLine();
                ...
                {
                    MailMessage m = new MailMessage(s.SiteEmailFromField, GetAdminRecipients());
                    m.Subject = "Form Notification Summary";
                    m.Body = messageBody.ToString();
                    SmtpClient client = new SmtpClient();
                    client.Send(m);
                }
